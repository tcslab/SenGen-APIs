...
{
	$allNodeNames=explode(",",mysqli_real_escape_string($conn,$_GET["node_name"]));
	foreach ($allNodeNames as $nodeName)
	{
		$allWheres[] ="measurements.node_name='".$nodeName."'";
	}
	$wheres[]="( ".implode(" or ",$allWheres)." )";
}
...
if (isset($_GET["before"]) || isset($_GET["after"]))
{
	if (isset($_GET["after"]))
	{
	  $wheres[]="(TIMESTAMPDIFF(SECOND, TIMESTAMP(\"".mysqli_real_escape_string($conn,$_GET["after"])."\"), TIMESTAMP(measurements.timestamp))>=0)";
	}
	  if (isset($_GET["before"]))
	  {
		  $wheres[]="(TIMESTAMPDIFF(SECOND, TIMESTAMP(\"".mysqli_real_escape_string($conn,$_GET["before"])."\"), TIMESTAMP(measurements.timestamp))<=0)";
	  }
}
